 ...
 */
void
listnode_add_first (struct list *list, void *val)
{
    struct listnode *node;
...
list_add_node_prev (struct list *list, struct listnode *current, void *val)
{
    ...
    if(list == NULL || current == NULL || val == NULL)
    {
        ERR_LOG ((int)list, (int)current, (int)val, 0);
        return;
    }
